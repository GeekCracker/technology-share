...
public class VRoleServiceImpl extends BaseServiceImpl<VRoleMapper, VRole> implements VRoleService {
    ...
    private List<Permission> permissionTree(VRole vRole){
        ...
        if(permissions != null && !permissions.isEmpty()){
            List<Permission> topPermission = permissions.stream().filter(item->item.getParentId()==null||item.getParentId()==0).collect(Collectors.toList());
    ...
    private void deep(List<Permission> treeList,List<Permission> all){
        for(Permission permission : treeList){
            List<Permission> children = all.stream().filter(item->permission.getIdRaw().equals(item.getParentId())).collect(Collectors.toList());
            if(!children.isEmpty()){
