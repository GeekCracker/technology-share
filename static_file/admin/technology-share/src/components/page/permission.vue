					...
					<el-main>
						<el-table v-loading="loading" :data="tableData" @selection-change="rowSelect" @select-all="selectAll"  border style="width: 100%;">
							...
							<el-table-column label="权限图标" width="80" align="center">
								<template slot-scope="scope">
									<i :class="scope.row.permissionIcon" style="font-size: 30px;"></i>
								</template>
							</el-table-column>
	...
	export default {
		...
		methods: {
			...
			reQuery() {
				...
				var vm = this;
				vm.ts.doPost(vm, '/admin/permission/queryPageData',{
					parentId: parentId,
					pageNum: pageNum,
					pageSize: pageSize
		          ...
		          beforeClose:function(action, instance, done){
		          	if (action === 'confirm') {
		             	...
		             	vm.instance=instance;
		              	vm.ts.doPost(vm, '/admin/permission/deleteById', {id:id}, null, function(vm, data) {
							if (data.data.code == 200) {
								vm.reQuery();
								vm.reQueryTree();
				...
				vm.$confirm('此操作将删除所有选中的权限, 是否继续?', '提示', {
		          ...
		          beforeClose:function(action, instance, done){
		          	if (action === 'confirm') {
		             	...
		             	vm.done = done;
		             	vm.instance=instance;
		              	vm.ts.doPost(vm, '/admin/permission/deleteBatchById',{deleteList:JSON.stringify(selections)}, null,null, function(vm, data) {
							if (data.data.code == 200) {
								vm.reQuery();
								vm.reQueryTree();
								vm.instance.confirmButtonLoading = false;
				...
				this.$refs[form].validate((valid) => {
					if (valid) {
						vm.ts.doPost(vm, '/admin/permission/save', formData, null, function(vm, data) {
							...
							vm.$message({message:data.data.msg,type:type});
						});
					} else {
						return false;
					}
